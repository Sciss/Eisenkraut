...
public class SuperColliderClient
    ...
    private final ServerOptions		so;
...
//		}
        ...
        try {
            if ((ggMain != null) && (server != null) && server.isRunning()) ggMain.free();
        } catch (IOException e1) {
        ...
        switch (e.getID()) {
            ...
            case ServerEvent.RUNNING:    // --------------------------------------- started
                ...
                try {
                    ...
                    bndl.addPacket(ggMain.addToTailMsg(grpRoot));
                    bndl.addPacket(grpGain		.addToHeadMsg(ggMain));
                    bndl.addPacket(grpLimiter	.addToTailMsg(ggMain));
